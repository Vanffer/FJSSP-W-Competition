...
def ecdf_inf(
    ...
    xlabel: str = None,
    ylabel: str = None,
    marker_frequence: int = 10,
    markers: list[str] = ["x", "o", "^", ">", "v", "<", "*"],
):
    # 绘制“允许 inf 缺失值”的经验累计分布曲线（ECDF）。
    plot_vectors = []
    ...
    for i in range(len(vectors)):
        if i >= len(markers):
            print(
                f"NOTE: not enough markers defined, recycling already used markers for {labels[i]}"
            )
        plt.plot(
            plot_vectors[i][0],
            plot_vectors[i][1],
            ...
            markevery=list(
                range(
                    0,
                    len(plot_vectors[i][0]),
                    max(1, int(len(plot_vectors[i][0]) / marker_frequence)),
                )
            ),
        )
...
def get_plot_vectors(
    data: dict[str, list[float]], delta_scope: float = 1.0
) -> tuple[list[float], list[str]]:
    # 将 {solver -> {instance -> fitness}} 转为可直接画 ECDF 的向量。
    plot_data = dict()
    best_results = dict()
    ...
    for key in data[example].keys():
        ...
        for solver in data.keys():
            if key in data[solver]:
                plot_data[key][solver] = max(
                    calculate_value(data[solver][key], best * delta_scope), 0
                )
            else:
...
def visualize_gaps(
    ...
    x_lim_lb=-0.1,
    x_lim_ub=1.75,
    delta_scope: float = 1.0,
) -> None:
    # 可视化各算法在全实例上的 gap 分布表现。
    ...
    ecdf_inf(
        ...
        x_lim=(x_lim_lb, x_lim_ub),
        xlabel="$\delta_{rel}$",
        ylabel="Portion of Instances $\leq\delta_{rel}$",
    )


def progress_plot(
    fitness_data: list[float],
    ...
    for i in range(len(labels)):
        ...
        plt.plot(
            ...
            label=[label],
            marker=markers[i % len(markers)],
            markevery=list(range(0, len(x), max(1, int(len(x) / marker_frequence)))),
        )
    if not xlim_ub:
        xlim_ub = n
    if hline:
...
def visualize_timeline(
    ...
    # 把时间序列原始数据转换为相对偏差后交给 progress_plot。
    fitness_data = []
    timestamps = []
    labels = []
    best = float("inf")
    ...
    for solver in data:
        labels.append(solver)
        fitness_data.append(
            [max(calculate_value(entry[1], best), 0) for entry in data[solver]]
        )
    ...
    progress_plot(
        ...
        xlim_ub=xlim_ub,
        hline=(
            calculate_value(best * (1 + (1.0 - delta_scope)), best)
            if delta_scope < 1.0
            else None
        ),
    )
...
def rank_plot(
    ...
    # 使用 autorank 输出 Friedman/Nemenyi 统计比较图。
    import pandas as pd
...
def show_simulation_results(instance: dict, results: list[float]):
    ...
    plt.hlines(
        statistics.mean(results), 0, len(results), color="red", label="Average Makespan"
    )
    plt.hlines(
    ...
    plt.title(
        f'Original (Planned) Makespan: {max(instance["e"])} | R: {statistics.mean(results)/max(instance["e"]):.4f}'
    )
    plt.legend()
    plt.show()


def show_simulation_comparison(
    results: list[list[float]],
    labels: list[str],
    instance: dict,
    title: str = None,
    mark_average: bool = False,
):
    # 对比多组仿真结果：左侧排序曲线，右侧箱线图。
    plt.rcParams["axes.grid"] = True
    plt.rcParams["pdf.fonttype"] = 42
    plt.rcParams["ps.fonttype"] = 42
    ...
    if mark_average:
        axs[0].scatter(
            [average[0] for average in averages],
            [average[1] for average in averages],
            s=20,
            marker="+",
            color="red",
            zorder=10000,
